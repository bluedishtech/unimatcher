...
const UserCard: React.FC<{ user: User }> = ({ user }) => (
  <div className="bg-white p-4 rounded-lg shadow-md flex items-center">
    <span className="material-symbols-outlined text-3xl text-indigo-500 mr-4">school</span>
    <div>
...
const Freshmen: React.FC = () => {
  const groupedUsers = useMemo(() => {
    const groups: { [date: string]: User[] } = {};
    const sortedUsers = [...freshmen].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-12">
        <h1 className="text-3xl font-bold text-indigo-600">Nouveaux Inscrits</h1>
        <p className="text-slate-600 mt-2">Une chronologie de tous les nouveaux étudiants inscrits.</p>
      </div>

      {Object.keys(groupedUsers).length > 0 ? (
        Object.entries(groupedUsers).map(([dateKey, usersOnDay]) => {
          const date = new Date(dateKey + 'T00:00:00'); // Use T00:00:00 to avoid timezone shifts
        ...
        <div className="text-center bg-white p-8 rounded-lg shadow-md">
            ...
            <p className="text-slate-500">Aucun nouvel étudiant ne s'est encore inscrit.</p>
        </div>
      )}
    </div>
  );
